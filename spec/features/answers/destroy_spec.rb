...
feature 'User can destroy answer', %q{
  ...
  feature "being authorized" do
    ...
    scenario "tries to delete other's question" do
      ...
      visit question_path(question)
      expect(page).to satisfy("No delete button for other's answers") do |page|
        page.find(:xpath, "//*[contains(text(), '#{other_answer.body}')]/parent::tr").has_no_button?("Delete answer")
      end
    end
    ...
    scenario "deletes own answer" do
      answer
      visit question_path(question)
      page.find(:xpath, "//*[contains(text(), '#{answer.body}')]/parent::tr").click_button("Delete answer") # find_button("#delete").click
